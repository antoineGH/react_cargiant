					...
					<Container>
						<Row className='justify-content-center'>
							{items.cars.map((item) => {
								...
								return (
									<Col key={item.id} xs={11} sm={6} md={4} lg={4} className='mt-4'>
										<Card key={item.id} className='justify-content-centers'>
											...
											<Card.Body>
												...
												<Card.Text>Matriculation: {item.matriculation}</Card.Text>
												<Card.Text className='font-weight-bold'>{item.price}€ TTC</Card.Text>
												<Button variant='primary'>
													Read More <FontAwesomeIcon size='xs' icon={['fas', 'greater-than']} fixedWidth />
												</Button>
											</Card.Body>
										</Card>
									</Col>
